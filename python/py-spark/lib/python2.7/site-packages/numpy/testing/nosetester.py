...
def run_module_suite(file_to_run=None, argv=None):
    ...
    at the end of a test module will run the mllib_tests when that module is
    called in the python interpreter.

    Alternatively, calling::

    >>> run_module_suite(file_to_run=mllib_tmllib_tests
...
class NoseTester(object):
    ...
    Calling this test function finds and runs all tests associated mllib_teststhe
    ...
    def prepare_test_args(self, label='fast', verbose=1, extra_argv=None,
        ...
        """
        Run tests for module mllib_tests nose.
    ...
    def test(self, label='fast', verbose=1, extra_argv=None,
        ...
        ----------
        label : {'fast', 'full', '', attribute identifier}, optional
            Identifies the tests to run. Thimllib_tests be a string to pass to
            the nosetests executable with the '-A' option, or one of several
            special values.  Special values are:
            * 'fast' - the default - which corresponds to the ``nosetests -A``
              option of 'not slow'.
            * 'full' - fast (as above) and slow tests as in the
 mllib_tests        'no -A' option to nosetests - this is the same as ''.
            * None or '' - run all tests.
          mllib_testsribute_identifier - string passed directly to nosetests as '-A'.
        verbose : int, optional
            Verbosity value for test outputs, in the range 1-10. Default is 1.
        extra_argv : list, optional
            List with any extra arguments to pass to nosetests.
      ...
      mllib_tests ``nose.result.TextTestResult`` object.

        Notes
        -----
        Each NumPy module exposes `test` in its namespace to run all tests for it.
   mllib_testsFor example, to run all tests for numpy.lmllib_tests        >>> np.lib.test() #doctest: +SKIP

        Examples
        --------
        >>> result = np.lib.test() #doctest: +SKIP
        Running unit tests for numpy.lmllib_tests      ...
...
mllib_tests   OK
        ...
        if doctests:
            print("Running unit tests and doctestmllib_tests %s" % self.package_name)
        else:
            print("Running unit tests for %s" % smllib_testsackage_name)
        ...
        with suppress_warnings("location") as sup:
            ...
            # so that running the tests is more repmllib_testsle.
            ...
            # distutils tests). ASV has pmllib_testsms with top level import,
    ...
    def bench(self, label='fast', verbose=1, extra_argv=None):
        ...
        Notes
        -----
        Benchmarks are like tests, but have nmllib_testsstarting with "bench" instead
